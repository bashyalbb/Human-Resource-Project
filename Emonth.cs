...
namespace HRMS
{
    public partial class Emonth : Form
    {
        public Emonth()
        {
            InitializeComponent();
        ...
        private void FillEmployeeListing()
        {
            ...
            sda.Fill(dt);
            emonth_employee_id.ValueMember = "employee_id";
            emonth_employee_id.DisplayMember = "employee_id";
            emonth_employee_id.DataSource = dt;
        ...
        {
            emonth_id.Text = "";
            emonth_employee_id.Text = "";
            emonth_month.Text = "";
            emonth_description.Text = "";
        }

        private void save_data_Click(object sender, EventArgs e)
        {
            DBConnection cn = new DBConnection();
            String SQL = "INSERT INTO `emonth` (`emonth_employee_id`, `emonth_month`, `emonth_description`) VALUES (@0, @1, @2);";
            MySqlCommand insertCommand = new MySqlCommand(SQL, cn.con);
            insertCommand.Parameters.AddWithValue("@0", emonth_employee_id.Text);
            insertCommand.Parameters.AddWithValue("@1", emonth_month.Text);
            insertCommand.Parameters.AddWithValue("@2", emonth_description.Text);
        ...
        private void data_listing_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                emonth_id.Text = row.Cells[0].Value.ToString();
                emonth_employee_id.Text = row.Cells[1].Value.ToString();
                emonth_month.Text = row.Cells[2].Value.ToString();
                emonth_description.Text = row.Cells[3].Value.ToString();
            }
        }
        ...
        private void update_data_Click(object sender, EventArgs e)
        {
            ...
            {
                DBConnection cn = new DBConnection();
                String SQL = "UPDATE `emonth` SET `emonth_employee_id` = @0, `emonth_month` = @1, `emonth_description` = @2 WHERE `emonth_id` = @3; ";
                MySqlCommand insertCommand = new MySqlCommand(SQL, cn.con);
                insertCommand.Parameters.AddWithValue("@0", emonth_employee_id.Text);
                insertCommand.Parameters.AddWithValue("@1", emonth_month.Text);
                insertCommand.Parameters.AddWithValue("@2", emonth_description.Text);
                insertCommand.Parameters.AddWithValue("@3", emonth_id.Text);
                insertCommand.ExecuteNonQuery();
                FillDataListing();
                clearForm();
        ...
        private void delete_data_Click(object sender, EventArgs e)
        {
            ...
            if (emonth_id.Text != "")
            {
                String SQL = "DELETE FROM emonth WHERE emonth_id = " + emonth_id.Text;
        ...
        private void emonth_description_TextChanged(object sender, EventArgs e)
        {
        ...
        private void Emonth_Load(object sender, EventArgs e)
        {
